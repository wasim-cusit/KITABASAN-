...
<head>
    ...
    <title>New Device Binding</title>
    <style>
...
<div class="container">
    <div class="header">
        <h1>New device bound</h1>
    </div>
    <div class="content">
        <p>A student has bound a device for the first time (or after reset).</p>
        <div class="box">
            ...
            <strong>Status:</strong> {{ ucfirst(str_replace('_', ' ', $binding->status ?? 'N/A')) }}<br>
            <strong>Bound at:</strong> {{ $binding->created_at?->format('M d, Y H:i') ?? 'N/A' }}
        </div>
        <p>View in admin: <a href="{{ route('admin.devices.index') }}">Device Management</a></p>
    </div>
    <div class="footer">
