...
<body>
    <div class="container">
        <div class="header">
            <h1>New enrollment in your course</h1>
        </div>
        <div class="content">
            <p>Hello {{ $teacher->first_name ?: $teacher->name }},</p>
            <p>A student has enrolled in your course.</p>
            <div class="box">
                <strong>Course:</strong> {{ $enrollment->book->title ?? 'N/A' }}<br>
                <strong>Student:</strong> {{ $enrollment->user->name ?? $enrollment->user->email }}<br>
                <strong>Enrolled at:</strong> {{ $enrollment->enrolled_at?->format('M d, Y H:i') ?? 'N/A' }}
            </div>
            <p>View your students: <a href="{{ route('teacher.students.index') }}">{{ route('teacher.students.index') }}</a></p>
        </div>
        <div class="footer">
