...
<head>
    ...
    <style>
        ...
        .box { background: #f5f3ff; border: 1px solid #c4b5fd; border-radius: 6px; padding: 14px; margin: 16px 0; font-size: 14px; }
...
<body>
    <div class="container">
        ...
        <div class="content">
            <p>A new student has joined the platform.</p>
            <div class="box">
                <strong>Name:</strong> {{ $student->name ?? 'N/A' }}<br>
                <strong>Email:</strong> {{ $student->email }}<br>
                <strong>Mobile:</strong> {{ $student->mobile ?? 'N/A' }}<br>
                <strong>Source:</strong> {{ $source === 'admin' ? 'Created by administrator' : 'Self registration' }}<br>
                <strong>Date:</strong> {{ $student->created_at?->format('M d, Y H:i') ?? 'N/A' }}
            </div>
            <p>View in admin: <a href="{{ route('admin.students.index') }}">Students</a></p>
        </div>
        <div class="footer">
