    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-4 lg:gap-6">
        ...
        <div class="lg:col-span-2">
            <div class="bg-white rounded-lg shadow p-4 lg:p-6 mb-6">
                <h1 class="text-xl lg:text-3xl font-bold mb-4">{{ $course->title }}</h1>
                ...
                @if($course->cover_image)
                    <img src="{{ \Storage::url($course->cover_image) }}" alt="{{ $course->title }}" class="w-full h-48 lg:h-64 object-cover rounded-lg mb-4">
                @endif
            </div>

            <!-- Course Content Preview -->
            <div class="bg-white rounded-lg shadow p-4 lg:p-6">
                <h2 class="text-xl lg:text-2xl font-bold mb-4">Course Content</h2>
        ...
        <div class="lg:col-span-1">
            <div class="bg-white rounded-lg shadow p-4 lg:p-6 lg:sticky lg:top-24">
                <div class="text-center mb-6">
                    @if($course->is_free)
                        <div class="text-2xl lg:text-3xl font-bold text-green-600 mb-2">Free</div>
                    @else
                        <div class="text-2xl lg:text-3xl font-bold text-blue-600 mb-2">Rs. {{ number_format($course->price, 0) }}</div>
                    @endif
                </div>

                @if($enrollment)
                    <a href="{{ route('student.learning.index', $course->id) }}"
                       class="block w-full bg-blue-600 text-white text-center px-4 lg:px-6 py-2 lg:py-3 rounded-lg hover:bg-blue-700 font-semibold mb-3 text-sm lg:text-base">
                        Continue Learning
                    </a>
                ...
                @else
                    @if($course->is_free)
                        <form action="{{ route('student.courses.enroll', $course->id) }}" method="POST">
                            @csrf
                            <button type="submit" class="w-full bg-green-600 text-white px-4 lg:px-6 py-2 lg:py-3 rounded-lg hover:bg-green-700 font-semibold mb-3 text-sm lg:text-base">
                                Enroll for Free
                            </button>
                        </form>
                    @else
                        <form action="{{ route('student.courses.enroll', $course->id) }}" method="POST">
                            @csrf
                            <button type="submit" class="w-full bg-blue-600 text-white px-4 lg:px-6 py-2 lg:py-3 rounded-lg hover:bg-blue-700 font-semibold mb-3 text-sm lg:text-base">
                                Purchase Course
                            </button>
                        </form>
                    @endif
